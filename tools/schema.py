...
def inject_schema(html_path, schema_blocks):
    ...
    # Replace the slot marker (multiple possible forms after renderer runs)
    html = re.sub(
        r'<!-- SLOT: schema_json_ld -->\s*(?:\{\{\s*schema_blocks\s*\}\}|<!-- CONTENT PENDING: schema_blocks -->)',
        f'<!-- Schema generated by schema.py -->\n  {schema_html}',
        html
    )

    # Also replace standalone forms
