...
def inject_schema(html_path, schema_blocks):
    ...
    # Replace the slot marker — handle all possible states after renderer
    # The slot comment may be followed by template tag, CONTENT PENDING, or nothing
    html = re.sub(
        r'<!-- SLOT: schema_json_ld -->[^\n]*',
        f'<!-- Schema generated by schema.py -->\n  {schema_html}',
        html
    )

    # Also replace any remaining standalone forms
    html = html.replace('{{ schema_blocks }}', schema_html)
    html = html.replace('<!-- CONTENT PENDING: schema_blocks -->', schema_html)
